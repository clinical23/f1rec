...
\`\`\`json
{
  ...
  "meta_description": "string — 140-160 chars, Google SERP-friendly",
  "category": "race-review | driver-analysis | season-preview | history | tech",
  "related_race_slug": "string|null — must exactly match a slug from the fact sheet",
  "related_driver_slug": "string|null — must exactly match a slug from the fact sheet",
...
If the fact sheet's \`race.season_year\` contradicts your instinct about the era, **trust the fact sheet.**
...
3. **At least 4 markdown internal links** using these exact patterns:
   - Drivers: \`[Full Name](/drivers/{driver_slug})\`
   - Teams: \`[Team Name](/teams/{team_slug})\`
...
\`\`\`json
{ "winner_name": "Kimi Antonelli", "grid": 2, "points": 25 }
\`\`\`
...
- \`content\`: 3000-5000 characters (roughly 500-800 words). Aim for the upper half — readers prefer depth.
- \`excerpt\`: 140-200 chars
- \`meta_description\`: 140-160 chars (Google SERP limit)
- \`title\`: 40-80 chars

Going outside these ranges returns validation errors. Don't.

---

## FINAL CHECKLIST (run through this before returning)
...
- [ ] \`related_race_slug\` exactly matches \`race.slug\` from the fact sheet, or is null
- [ ] \`related_driver_slug\` exactly matches a driver_slug from the fact sheet, or is null
